...
public class AiPlayer extends Player {
    ...
    //MCTS Algorithm and Utility Classes
    ...
    private static class MCTS {
        ...
        private Node expandNode(Node selectedNode) {
            ...
            boolean gameStatus = isTheGameOngoing(selectedNode.getBoard());
            if (!gameStatus) {
                return selectedNode;
    ...
    public static Node findBestNodeWithUCT(Node node) {
        ...
        for (Node child : children) {
            ...
            if (nodeVisit == 0) {
                return child;   // Return the node immediately if it's not visited yet
            }

            double uctValue = (nodeWinScore / (double) nodeVisit) + 1.41 * Math.sqrt(Math.log(parentVisit) / (double) nodeVisit);
